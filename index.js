...
function getFlavorByIndex(flavorsArray, indexNumber){
  return flavorsArray[indexNumber];
}
...
function removeFlavorByName(flavorsArray, flavor){
  for(let i = 0; i < flavorsArray.length; i++) {
    if(flavorsArray[i] === flavor) {
...
function filterByWord(flavorsArray, promotional){
  const filteredArray = [];
  for(let i = 0; i < flavorsArray.length; i++) {
    if(flavorsArray[i].includes(promotional)) {
      filteredArray.push(flavorsArray[i]);
    } else {
      continue;
    }
  }
...
function getAverageWordLength(array){
  ...
  const average = Math.round(wordCount / elementCount);
  return average;
}
...
function getRandomFlavors(menu1, menu2, menu3, menu4) {
  const menusArray = [menu1, menu2, menu3, menu4];
  const randomFlavors = [];
  console.log(menusArray[1]);

  for(let i = 0; i < MENULENGTH; i++) {
    let menu = Math.ceil(Math.random() * 4);
    let flavor = Math.ceil(Math.random() * menusArray[menu - 1].length);
    randomFlavors.push(menusArray[menu - 1][flavor - 1]);
  }
